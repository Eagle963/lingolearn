...
import { Heart, Flame, Trophy, Zap, Target, Award, Home, User, RotateCcw, Lock, Check, X, ChevronRight, ChevronLeft, Gift, Sparkles, BookOpen, MessageCircle, Plane, Utensils, ShoppingBag, Briefcase, Code, Mail, PenTool, HelpCircle, Globe, Headphones } from 'lucide-react'
...
export default function LingoLearnApp() {
  // Navigation & UI State
  const [screen, setScreen] = useState<'home' | 'lesson' | 'result' | 'profile'>('home')
  ...
  // NAVIGATION BAR
  const NavBar = () => (
    <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 px-8 py-3 flex justify-around items-center z-30 shadow-lg max-w-md mx-auto">
      <button 
        onClick={() => setScreen('home')} 
  ...
  return (
    <div className="relative">
      ...
      {(screen === 'home' || screen === 'profile') && <NavBar />}
    </div>
  )
}
